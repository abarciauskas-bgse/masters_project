...
def train(data, num_classes, max_iters, batch_size = 25, sample = False, store = False):
    reset_params()
    if store == True:
        print 'here'
        fo = open("weights.txt", "wb")
        fo.write( "Wxh0,Wh0h0,bh0,Wh0l0,bl0,Wl0h1,Wh1h1,bh1,Wh1y,by\n")
    losses = []
    ...
    for i in range(max_iters):    
        ...
        for param, dparam, mem in zip([Wxh0,  Wh0h0,   bh0,  Wh0l0,  bl0,  Wl0h1,  Wh1h1,  bh1,  Wh1y, by], 
            ...
            params_as_string += str(param) + ","
        if store == True:
            params_as_string = params_as_string[0:-1]
            params_as_string += "\n"
            fo.write(params_as_string)
        smooth_loss = smooth_loss * 0.999 + loss * 0.001
        losses.append(smooth_loss)
        pointer += batch_size
    if store == True: fo.close()
